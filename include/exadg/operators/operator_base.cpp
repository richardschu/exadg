...
OperatorBase<dim, Number, n_components>::apply_add(VectorType & dst, VectorType const & src) const
{
  ...
  else
  {
    ...
    {
      matrix_free->loop(
        &This::cell_loop, &This::face_loop_empty, &This::boundary_face_loop_hom_operator, this, dst, src);
    }
    else
    {
      matrix_free->cell_loop(&This::cell_loop, this, dst, src);
                                                      ...
                                                      VectorType const & src) const
{
  ...
  {
	  matrix_free->loop(
		&This::cell_loop, &This::face_loop_empty, &This::boundary_face_loop_full_operator, this, dst, src);
  }
}
...
OperatorBase<dim, Number, n_components>::add_diagonal(VectorType & diagonal) const
{
  ...
  if(evaluate_face_integrals())
  {
	...
	{
	  AssertThrow(not data.use_cell_based_loops, dealii::ExcMessage("Cell-based loops not available for CG."));
	  matrix_free->loop(&This::cell_loop_diagonal,
						&This::face_loop_empty,
						&This::boundary_face_loop_diagonal,
						this,
  ...
  Range const &                           range) const
{
  ...
  else
  {
	  for(unsigned int face = range.first; face < range.second; face++)
	  {
		...
		do_boundary_integral_continuous(*integrator_m, OperatorType::full, matrix_free.get_boundary_id(face));
  ...
  Range const &                           range) const
{
  ...
  {
	// do nothing, face loops do not contribute.
  }
}
