      ...
      constraints)
{
  ...
  else
  {
    ...
    std::vector<dealii::types::global_dof_index> mg_dof_indices(dof_handler.get_fe().dofs_per_cell);
    const dealii::IndexSet &                     active = dof_handler.locally_owned_dofs();
    const dealii::IndexSet & level = dof_handler.locally_owned_mg_dofs(level_info.back().h_level());
