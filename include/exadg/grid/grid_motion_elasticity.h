         ...
         types::time_step time_step_number) override
  {
    ...
    if(param.large_deformation) // nonlinear problem
    {
      ...
      auto const iter = pde_operator->solve_nonlinear(
        displacement, const_vector, 0.0 /* no mass terms */, 0.0 /* no mass terms */, time, update_preconditioner);
    ...
    else // linear problem
    {
      ...
      auto const iter = pde_operator->solve_linear(
        displacement, rhs, 0.0 /* no mass terms */, 0.0 /* no mass terms */, time, false /* do not update preconditioner */);
