...
public:
    ...
    unsigned int const              level = dealii::numbers::invalid_unsigned_int)
  {
    ...
    // Set up `dealii::FEValues` with `dealii::FE_Nothing` and the Gauss-Lobatto quadrature to
    // reduce setup cost, as we only use the geometry information (this means we need to call
    // `fe_values.reinit(cell)` with `Triangulation::cell_iterator` rather than the more standard
    // call using `dealii::DoFHandler::cell_iterator`).
    dealii::FE_Nothing<dim>                fe_nothing;
    std::shared_ptr<dealii::FEValues<dim,dim>> fe_values;
    if(mapping != nullptr)
    {
      fe_values = std::make_shared<dealii::FEValues<dim,dim>>(*mapping,
                                                          fe_nothing,
